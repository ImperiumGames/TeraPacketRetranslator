...
namespace TeraPacketRetranslator.Extractor
{
    public class MessageSplitter
    {
        ...
        {
            OnMessageReceived(new RawMessage(_time, MessageDirection.ClientToServer, new ArraySegment<byte>(block)));
        }

        private void ServerBlockFinished(byte[] block)
        {
            OnMessageReceived(new RawMessage(_time, MessageDirection.ServerToClient, new ArraySegment<byte>(block)));
        }

        protected void OnMessageReceived(RawMessage message)
        {
